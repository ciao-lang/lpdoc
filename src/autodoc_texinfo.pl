...
autodoc_is_operational_hook(texinfo, Alt) :-
    find_all_execs(~alt_requires_cmd(Alt)).
...
finish_texinfo :-
    ...
    %
    warn_on_nosuccess(copy_file(InInfodir, OutInfodir, [overwrite])).
...
:- use_module(library(system_extra), [del_files_nofail/1]).
