...
@lib{doccfg}. The second line defines through @pred{doc_structure/1}
the @index{document structure}, specifying in a tree the main file and
(optional) component files. When documenting complex manuals with more
than one source files, the main file is the one that will provide the
general title, author, date, summary, and the introduction. The
component files will appear as separate chapters.
...
@item @pred{filepath/1}: option to include all additional directories
  where the files to be documented can be found (@concept{alias path}s
  for libraries are included automatically).

@item @pred{doc_mainopts/1}: control what is included in the documentation
  for the main file.

@item @pred{doc_compopts/1}: sets options for the component files.

@item @pred{docformat/1}: determines the set of formats (@tt{pdf},
  @tt{ascii}, @tt{html}, @tt{info}, @tt{manl}, ...) in which the
  documentation should be generated by default.

@item @pred{output_name/1}: determines the base file name of the main
  documents generated by @apl{lpdoc}.

@item @pred{index/1}: determines the list of indices to be included at
  the end of the document.

@item @pred{bibfile/1}: determines a list of @index{.bib files}
  containing @index{bibliographic entries} for @concept{using
  citations}.

@item @pred{startpage/1}: allows changing the page number of the first
  page of the manual.

@item @pred{papertype/1}: allows select several paper sizes for the
  printable outputs (@tt{pdf}).
...
For each a @tt{.pl} file, @apl{lpdoc} tries to determine whether it is
a @index{library} or an @index{application} (exporting @pred{main/0}
or @pred{main/1}), and documents it accordingly.

The generated documentation for libraries will contain information on
the interface (e.g., the predicates exported by the file, the name of
the module and usage if it is a module, etc.), in addition to any
other machine readable comments included in the file. The interface
information is omitted in for applications by default.

Any combination of libraries and/or main files of applications can be
used arbitrarily as components or main files of a @apl{lpdoc}
...
In large documents, it is sometimes convenient to build a
super-structure of parts, each of which groups several
components. There is a special value of the second argument of the
@tt{:- doc(filetype,...).}  declaration mentioned above designed for
this purpose. The special @em{filetype} value @tt{part} can be used to
flag that the file in which it appears should be documented as the
start of one of the major @index{parts in a large document}. In order
to introduce such a part, a @tt{.pl} file with a declaration @tt{:-
doc(filetype,part).}  should be inserted in the sequence of files that
make up the @tt{components} variable of the @index{documentation
configuration} file at each point in which a major part starts. The
@tt{:- doc(title,""..."").}  declaration of this file will be used as
the part title, and the @tt{:- doc(module,""..."").}  declaration text
